...
<script language="JavaScript" type="text/JavaScript">
		...
		$(window).load(function(){
			...
			$.ajax({
				...
				data : "",
				dataType: "json",
				url: "/freebill_main_ajax",
				success: function(result) {
					if(result.returnCode == 'error'){
						$(".rem_con2 span").text(result.returnMsg);
					}else{
					    $(".rem_con2").hide();
					    $(".rem_con1").show();
					    $("#freebillDetailBtn",mainObj).attr("onClick", "goFreebillDetail();");
					    residuumTelMessage(result);
					}
				},
...
// 		}
		...
		//잔여기본통화 응답값 처리
	    function residuumTelMessage(result){
			
	   	  removeMessage(true);
	   	  
	   	  requestURL = 'N';
	   	  var prodNm = result.resultInfo.prodNm;
	   	  var prodId = result.resultInfo.prodId;
	   	  $("#p_Charge",mainObj).html("<a href='javascript:alert(\"상품 상세\");' ga:ca='MTA_main' ga:ac='상단메뉴' title="+prodNm+">"+prodNm+"</a>");
	 	 /**
	   	  * - 조회실패시  메세지 출력
	   	  * - 세션종료시 로그인 페이지로 이동처리 
	   	  */
		   ...
		   if(!D_Check){
		    ...
		    //toggleMsg(EVENT_TYPE_02, MSG_LOAD_TYPE_02, '<%= isLogin ? "Y" : "N" %>');
	   }
		...
		function sign(callType){
			 var resultCode =  1;
		 	 if(resultCode != 0){
		   	  	 ...
		   	  	 //미인증 회원 메세지 처리
		   	  	var resultMessage = "지금 로그인 하시면<br><span style='color:red;'>잔여기본통화</span>를 바로 조회하실 수 있습니다.";
		   	  	 
				//잔여기본통화
				if(callType == EVENT_TYPE_02){
					resultMessage += "<div class='n_btn'>";
					resultMessage += "	<a href=\"javascript:alert('로그인');\" class='btn2' target='_top' title='로그인 바로가기'>로그인</a>";
					resultMessage += "	<a href=\"javascript:alert('회원가입');\" class='btn2' target='_top' title='회원가입 바로가기'>회원가입</a>";
					resultMessage += "</div>";
				}
				
		   	  	//toggleMsg(callType, resultMessage, '<%= isLogin ? "Y" : "N" %>');
...
<body>
	<div class="rem_con1" style="display:none;">
		<a href="/freebill_detail.do" target="_top" class="data" id="d_call" title="잔여기본통화 바로가기">
			<span id="d_con" style="display:block">
			<span class="txt"><em id="d_call_h4"></em></span>
			<div class="graph"><span class="graph_bar" id="graph_bar3" style="width:50%;"></span></div><!-- style="width:;" bar퍼센트 -->
			<span class="remain">잔여</span>
				<strong class="volume" id="d_call_remain"></strong>
			</span>
		</a>	
						
		<a href="/freebill_detail.do" target="_top" class="pic" id="v_call" title="잔여기본통화 바로가기">
			<span id="v_con" style="display:block">
			<span class="txt"><em id="v_call_h4"></em></span>
			<div class="graph"><span class="graph_bar" id="graph_bar1" style="width:30%;"></span></div><!-- style="width:;" bar퍼센트 -->
			<span class="remain">잔여</span>
				<strong class="volume" id="v_call_remain"></strong>
			</span>
		</a>
							
		<a href="/freebill_detail.do" target="_top" class="sms" id="sms_mms" title="잔여기본통화 바로가기">
			<span id="s_con" style="display:block">
				<span class="txt"><em id="sms_mms_h4"></em></span>
	...
	<div class="rem_con2" style="text-align: center;" >
		<span style="height:30px;line-height: 100px;">조회중 입니다.</span>
	</div>
</body>
</html>